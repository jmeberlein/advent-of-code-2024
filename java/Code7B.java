    ...
    public static boolean check(List<Long> line) {
        ...
        while (!targetStack.empty()) {
            ...
            if (index == 1 && target == line.get(1)) {
                ...
                if (target % Math.pow(10, Math.ceil(Math.log10(line.get(index) + 1))) == line.get(index)) {
                    targetStack.push(target / (long)Math.pow(10, Math.ceil(Math.log10(line.get(index) + 1))));
                    indexStack.push(index - 1);
                }
            }
        }
